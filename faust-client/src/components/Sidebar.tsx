  ...
  return (
    <Link
      ...
      className={`flex items-center gap-3 px-4 py-2.5 rounded-sm transition-all duration-300 group relative overflow-hidden ${
        isActive
          ? isOrange ? 'bg-orange-500/10 text-orange-500' : 'bg-brand-accent/10 text-brand-accent'
          : 'text-slate-500 hover:text-slate-200 hover:bg-white/5'
      }`}
    >
      {isActive && (
        <div className={`absolute left-0 top-0 bottom-0 w-[2px] ${isOrange ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)]' : 'bg-brand-accent shadow-[0_0_8px_rgba(59,130,246,0.8)]'}`} />
      )}
